	...
	$("#searchbox").autocomplete({
		...
		select : function(event, ui) {
			...
			setTimeout(function() {
				...
				$('li[data-id="' + ui.item.id + '"]').scrollintoview({
					duration : "slow"
				});
...
function loadItem(id) {
	$('.loadingItem').show();
	$.get(OC.generateUrl('apps/passman/api/v1/item/' + id), {
		'id' : id
	}, function(data) {
		$('#id_files').html('');
		$('#customFieldsTable').html('');
...
function saveItem() {
	...
	$.each($('#existingFields tr'), function() {
		...
		customFields.push({
			id : fieldId,
			name : encryptThis(fieldName),
			value : encryptThis(fieldValue)
		});
	});
...
function editItem(itemId) {
	...
	$.get(OC.generateUrl('apps/passman/api/v1/item/' + itemId), {
		'id' : itemId
	}, function(data) {
		var item = data.item;
...
function loadFile(fileId) {
	$.get(OC.generateUrl('/apps/passman/api/v1/item/file/' + fileId), {
		'fileid' : fileId
	}, function(data) {
		console.log(data);
